...
def scan_log_file(log_path: str) -> list:
    ...
    for ip in unique_ips:
        ...
        if risk_data:
            hits.append({
                "ip": ip,
                "score": risk_data["abuse_confidence_score"],
                "reports": risk_data["total_reports"],
                "country": risk_data["country_code"],
                "source": "local_db"
            })
            print(f"🔴 HIT: {ip} (score={risk_data['abuse_confidence_score']})")
